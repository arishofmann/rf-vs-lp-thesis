"""Experiment runner: LP / RF / kNN on TerraMind v1 embeddings, scenarios FI/PT.

Runs experiments 0 (pre-study, all 19 classes), A (forest subset), B (sample
efficiency), C (RF tuning), D (per-class shift), E1 (native multi-output RF)
and E3 (LP on PCA features). Outputs results/results_experiment_{tag}.{csv,json}.

Usage:
    python run_all_experiments_v2.py [--experiments 0 A B C D E1 E3]
"""
...
def subsample_stratified(X, y, fraction, seed):
    """Stratified subsample: >=1 positive per class, then uniform fill."""
    if fraction >= 1.0:
        return X, y
...
def _rf_native_predict_proba(rf, X_test, num_classes):
    """Unpack sklearn's per-label proba list into (n, num_classes) array."""
...
def experiment_B(base_dir, output_dir):
    """Sample efficiency with stratified subsampling."""
...
def experiment_C(base_dir, output_dir):
    """RF optimisation: balanced, mtry-100/200, PCA-50/100/200."""
    ...
    for sc_name, (train_c, test_c) in SCENARIOS.items():
        ...
        for seed in SEEDS:
            # --- RF variants on raw 768-dim embeddings ---
...
def experiment_E1(base_dir, output_dir):
    """E1: native multi-output RF (without Binary Relevance wrapper)."""
...
def experiment_E3(base_dir, output_dir):
    """E3: Linear Probing on PCA-reduced embeddings."""
...
def main():
    ...
    parser.add_argument("--experiments", nargs="+",
                        default=["0", "A", "B", "C", "D"],
                        help="Which experiments to run (0, A, B, C, D, E1, E3, E4)")
